 ...
 * BestcyclingWebview
 */
public class BestcyclingWebview implements MethodCallHandler, PluginRegistry.ActivityResultListener {
  private Activity activity;
  private WebviewManager webViewManager;
  private Context context;
  static MethodChannel channel;
  private static final String CHANNEL_NAME = "bestcycling_webview";

  public static void registerWith(PluginRegistry.Registrar registrar) {
    channel = new MethodChannel(registrar.messenger(), CHANNEL_NAME);
    final BestcyclingWebview instance = new BestcyclingWebview(registrar.activity(), registrar.activeContext());
    registrar.addActivityResultListener(instance);
    channel.setMethodCallHandler(instance);
  }

  private BestcyclingWebview(Activity activity, Context context) {
  ...
  private void openUrl(MethodCall call, MethodChannel.Result result) {
    ...
    webViewManager.openUrl(withJavascript, clearCache, hidden, clearCookies, userAgent, url, headers, withZoom,
        ...
        invalidUrlRegex, geolocationEnabled);
    result.success(null);
  }
